...
//	public static long confinedNeighbors;
	...
	//public static Map <pair, Vector<Vector<Integer>>> map = new HashMap<pair, Vector<Vector<Integer>>>();
	public static Map <pair, Vector<Integer>> map = new HashMap<pair, Vector<Integer>>();



        //pruning techniques
        public int[] prevSize;
        public VCSolver[] prevSubSolvers;
	...
	public static boolean newSetMode = false;
//        public static long counterRestore = (long)0;
//        public static long counterLowerBound = (long)0;
//        public static long counterDecomp = (long)0;
//        public static long beginDecomp = (long)0;
//        public static int counterDecomposeMatch = 0;
//        public static int counterDecomposeTotal = 0;
...
//                            deg[v] = n == rn ? adj[v].length : deg(v); no need to use deg[] now
                    deque.add(v);
            }
        }
...
//							this.repeatUnconfined++;
	...
	void branching() {
		...
		restore(pn);
//                counterRestore += (System.currentTimeMillis() - beginCounter);
		if (lb >= opt) {
//                    counter += System.currxentTimeMillis() - beginCounter;
                    return;
                }
		nBranchings++;
		if (mirrorN == 0) {
...
//		long beginCounter2 = System.currentTimeMillis();
		restore(pn);
//                counterRestore += (System.currentTimeMillis() - beginCounter2);
	}
	...
	boolean decompose() {
		int[][] vss;
                VCSolver[] newVCS = new VCSolver[n];
                int[] reverseMap;
		try (Stat stat = new Stat("decompose")) {
			...
			int nC = 0;
			{
                for (int i = 0; i < n; i++) id[i] = -1;//id[] stores all vertices as -1
                for (int s = 0; s < n; s++) if (x[s] < 0 && id[s] < 0) {//all indetermined vertices
                        nC++;//dont konw what yet
                        int qs = 0, qt = 0;
                        que[qt++] = s;//store s in queue
                        id[s] = s;//set id[s] from -1 to s
                        while (qs < qt) {
                                int v = que[qs++];//v is equal to s
                                for (int u : adj[v]) if (x[u] < 0 && id[u] < 0) {
                                        id[u] = s;//making all the connected parts the same ID
                                        que[qt++] = u;//adding u to que
                                }
                        }
                        boolean identical = true;

                        if (prevSubSolvers != null && prevSubSolvers[s] != null 
                                && prevSize[s] == qt && map.get((nC,s)) != null) {

                        	//boolean [] checklist = new boolean[rn];
                        	Vector<int> graph_tb_added;
                        	// for (int y = 0; y < rn; y ++)
                        	// 	checklist[y] = false;

                        	// for (int j = 0; j < map.get((rn,s)).length; j++){
                        	// 	for (int k = 0; k < adjacents.length; k++){
                        	// 		// if (id[k] != -1)
                            // 			if (x[k] < 0 && adjacents[k].length == map.get((rn,s))[j].length){
                            // 				checklist[j] = true;
                            // 				graph_tb_added.add(graph_tb_added.length ,adjacents[k]);
                            // 				continue;

                            // 			}
                        	// 	}
                        	// }
                        	Vector a = map.get((nC,s));
                        	int j = 0;
                    		for (int k = 0; k < adjacents.length; k++){
                    			if (id[k] != -1){
                    				if (k != a[j]){
                        ...
                        if (identical){
                            	newVCS[s] = this.prevSubSolvers[s];
                        	}
                        else{
                        	Vector<int> graph_tb_added;
                        	for (int y = 0; y < n; y++){
                        		if (id[y] != -1){
                        			graph_tb_added.add(graph_tb_added.size(), y)
                        			for (int p = 0; p < adjacents[y].length){
                        				graph_tb_added.add(graph_tb_added.size(), adjacents[y][p])
                        			graph_tb_added.add(graph_tb_added.size(), -1);
                        			}
                        		}
                        	}
                        	map.put(new pair(nC,s), graph_tb_added);
                        }
			...
			long[] cs = new long[nC];
			{
                            ...
                            for (int i = 0; i < n; i++) if (x[i] < 0 && id[i] == i) {
                                    cs[p++] = ((long)(size[i])) << 32 | i;/*getting the size of each component
                                    first half stores the size, second half stores the id 
                                    */
                            }
                            sort(cs);//sort based 
			}
			...
			vss = new int[nC][];//nc represents number of components
                        reverseMap = new int[nC];
			int[] qs = new int[n];
			{
				for (int i = 0; i < nC; i++) {
					vss[i] = new int[size[(int)cs[i]]];//each i is the id of component(0,1,2)
					qs[(int)cs[i]] = i;/*qs index is the id of component and value is the index 
                                        of component[.1.4..7]
                                        */
				}
				int[] ps = new int[nC];
				for (int i = 0; i < n; i++) if (x[i] < 0) {
                                        int j = qs[id[i]];//index of component in qs, could be 0, 1, 2...  
                                        reverseMap[j] = id[i];
					vss[j][ps[j]++] = i;/*
                                        the columns of vss stores v that are connected to certain component id
                                        */
				}
			}
			for (int i = 0; i < n; i++) id[i] = -1;//reset id
			for (int i = 0; i < vss.length; i++) {
				int[] vs = vss[i];//each component's vertices  
				long[] ls = new long[vs.length];//an array stores long
				for (int j = 0; j < vs.length; j++) ls[j] = ((long)(n - deg(vs[j]))) << 32 | vs[j];
                                /*
                                stores n - deg(v)?
                                */
				sort(ls);
				for (int j = 0; j < vs.length; j++) vs[j] = (int)ls[j];//restore back to vs[j] from vss
			}
		}
		int[] x2 = x.clone();
...
//                counterDecomp += System.currentTimeMillis() - beginDecomp;//test runtime of decompose()
		VCSolver[] vcs = new VCSolver[vss.length];//test runtime of decompose()
//                beginDecomp = System.currentTimeMillis();//test runtime of decompose()
		{
			for (int i = 0; i < vss.length; i++) {
				int[] vs = vss[i];
				size[i] += 2; 
		...
		int[][] vss2 = new int[vss.length][];
		{
			...
			for (int i = 0; i < N; i++) if (pos1[i] >= 0) {
                            vss2[pos1[i]][pos2[i]] = i;
                        }

		}
		int sum = crt;
                mainLoop : for (int i = 0; i < vss.length && opt > sum; i++) {
//                        this.counterDecomposeTotal++;
                        VCSolver vc = vcs[i];
                        boolean changed = false;
                        if (newVCS[reverseMap[i]] != null) {
                            secondLoop : for (int k = 0; k < vc.adj.length;k++) {
                                if (vc.adj[k].length != newVCS[reverseMap[i]].adj[k].length) {
                                    changed = true;
                                    break;
                                }
                                for (int l = 0; l < vc.adj[k].length; l++) {
                                    if (vc.adj[k][l] != newVCS[reverseMap[i]].adj[k][l]) {
                                        changed = true;
                                        break secondLoop;
                                    }
                                }
                            ...
                            if (changed == false) {
//                                this.counterDecomposeMatch++;
                                vc.opt = newVCS[reverseMap[i]].opt;
                                vc.y = newVCS[reverseMap[i]].y;
                                sum += vc.opt;
	...
	boolean reduce() {
		...
		for (;;) {
			...
			if (n > 100 && n * SHRINK >= rn && !outputLP) {
//                            beginDecomp = System.currentTimeMillis();
                            boolean result = decompose();
//                            counterDecomp += (System.currentTimeMillis() - beginDecomp);
                            if (result) return true;
                            //to test how long it takes for decompose()
                        }
	...
	void rec() {//RECURSION
		...
		if (reduce()) return;
                ...
                if (lowerBound() >= opt) {
//                    counterLowerBound += (System.currentTimeMillis() - begin);
                    return;
                }
		if (rn == 0) {
